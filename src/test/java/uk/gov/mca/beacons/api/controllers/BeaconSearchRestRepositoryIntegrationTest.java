...
class BeaconSearchRestRepositoryIntegrationTest {
    ...
    @Test
    void givenAccountHolderHasClaimedLegacyBeacon_ThenItShouldBeReturnedAsANormalRegistrationNotALegacyBeacon() throws Exception {
        // Setup
        ...
        final var legacyBeaconId = testSeeder.getLegacyBeaconId(legacyBeaconResponseBody);
        final var createAccountResponseBody = testSeeder.seedAccountHolderWithEmail(email);
        final var accountHolderId = testSeeder.getAccountHolderId(createAccountResponseBody);

        final Object requestBody = testSeeder.registrationFixtureToMap(
                        testSeeder.readFile("src/test/resources/fixtures/registrations.json")
                                .replace("replace-with-test-account-holder-id", accountHolderId)
                )
                .get(RegistrationsControllerIntegrationTest.RegistrationUseCase.SINGLE_BEACON);
    ...
    private void createLegacyBeacon(Function<String, String> mapRequestObject)
        ...
        final var createLegacyBeaconRequest = mapRequestObject.apply(
                testSeeder.readFile("src/test/resources/fixtures/createLegacyBeaconRequest.json")
        );
    ...
    class GetBeaconSearchResults {
        ...
        void shouldFindTheCreatedLegacyBeacon() throws Exception {
            ...
            testSeeder.seedLegacyBeaconWithEmailAndHexId("ownerbeacon@beacons.com", randomHexId);
        ...
        void shouldFindTheCreatedBeacon() throws Exception {
            final var randomHexId = UUID.randomUUID().toString();
            testSeeder.seedBeacon(request -> request.replace("1D0EA08C52FFBFF", randomHexId));
    ...
    class GetBeaconSearchResultsForAccountHolder {
        ...
        void shouldNotFindAnyBeaconsIfEmptyQueryParamsSubmitted() throws Exception {
            testSeeder.seedBeacon(request -> request.replace("\"account-holder-id-placeholder\"", "null"));
        ...
        void shouldFindTheBeaconByAccountHolderId() throws Exception {
            final var accountHolderId = testSeeder.getAccountHolderId(testSeeder.seedAccountHolderWithAuthId(
                    UUID.randomUUID().toString()
            ));
            testSeeder.seedBeacon(request ->
